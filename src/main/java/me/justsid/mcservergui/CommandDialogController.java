...
public class CommandDialogController {
    ...
    private void loadCommandDetails(String displayText) {
        ...
        for (ScheduledCommand cmd : commandManager.getCommands()) {
            if (cmd.getDisplayText().equals(displayText)) {
                ...
                ScheduledCommand.TriggerType triggerType = cmd.getTriggerType() != null
                        ? cmd.getTriggerType()
                        : ScheduledCommand.TriggerType.PLAYER_COUNT_ZERO;
    ...
    private void onUpdateClick() {
        ...
        if (command.length() > MAX_COMMAND_LENGTH) {
            showAlert("Fehler", "Befehl ist zu lang (max. " + MAX_COMMAND_LENGTH + " Zeichen).");
            return;
        }
            ...
            switch (triggerType) {
                ...
                case EVERY_X_MINUTES:
                    if (!conditionField.getText().isEmpty()) {
                        ...
                        if (playerCount < 0 || playerCount > MAX_PLAYER_COUNT) {
                            showAlert("Fehler", "Spielerzahl muss zwischen 0 und " + MAX_PLAYER_COUNT + " liegen.");
                            return;
                        }
                    }
                    break;
                case AT_TIME:
                    ...
                    if (time != null && !time.isEmpty() && !TIME_PATTERN.matcher(time).matches()) {
                        showAlert("Fehler", "Ungültiges Uhrzeitformat. Bitte HH:mm verwenden (z.B. 14:30).");
                        return;
                    }
                    break;
            }
